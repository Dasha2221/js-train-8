...
function getUsersFromDatabase(array) {
  const result = Array.from(array, (user) => ({
    id: user.id,
    firstName: user.name.toUpperCase(), // Перетворюємо name в firstName верхнього регістру
    years: user.age, // Вікові дані залишаємо як years
  }));
  return result; // Повертаємо новий масив користувачів
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  arr.copyWithin(target, start, end + 1);
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr) || start < 0 || end > arr.length) {
    return false;
  }
